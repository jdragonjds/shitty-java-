...
public class StudentService {
    ...
    public void deleteStudent(Long studentId) {
        ...
        if(!exists)throw new IllegalStateException("student with id :"+studentId+" dosen't exist");
        studentRepository.deleteById(studentId);
    }
    @Transactional
    public void updateStudent(Long studentId, String name, String email) {
        Student student = studentRepository.findById(studentId).orElseThrow(()-> new IllegalStateException(
                "student with id :"+studentId+" dosen't exist"
        ));
        ...
        if(email!=null && email.length()>10 && !Objects.equals(student.getEmail(),email)) {
            ...
            if(studentOptional.isPresent())throw new IllegalStateException("email taken already");
